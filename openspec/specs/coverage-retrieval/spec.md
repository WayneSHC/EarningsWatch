# coverage-retrieval Specification

## Purpose

定義 `retrieve_coverage()`（`src/core/retriever.py`）「coverage sweep」補洞機制的可配置行為：當 LangGraph `retrieve` 節點的初步 top-k 結果遺漏某些季度時，coverage sweep 會以相似度門檻過濾後補抓。本 spec 規範該門檻的配置介面、預設行為、fallback 策略，以確保不同資料品質情境下可快速 tune 而不需改 code。
