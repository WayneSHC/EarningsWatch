...
Single `chat(prompt, max_tokens, mode)` entrypoint. Backend selected by `LLM_BACKEND` env var; auto-detects from available API keys in order: `anthropic → gemini → openai → groq → cohere`. `mode="dev"` uses cheaper/faster models; `mode="demo"` uses best quality.
...
OPENAI_API_KEY=...
GROQ_API_KEY=...
COHERE_API_KEY=...       # also used for Rerank
